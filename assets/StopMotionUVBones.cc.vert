...
void main(void){
    ...
    //Floor to be an integer step. Default time is in seconds, which we modify with speed. So, speed of 10 gives 10 steps per second; each step is a fixed size of offsetX/Y.
    float time = floor(CC_Time.y * steps);
    v_texCoord.x += mod(time * offsetX, 1.0);
    /*
    if(offsetX > 0.0) {
    }
    */
    ...
    if(offsetY > 0.0) {
        v_texCoord.y += mod(time * offsetY, 1.0);
    }
