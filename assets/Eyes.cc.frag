...
#if defined(USE_DIFFUSE) || defined(USE_EMISSIVE)
...
void main()
{
    ...
    float zFactor = smoothstep(0.2, 1.0, headNorm.y);
    ...
    //Calculate emissive component. The eye texture packs both diffuse and emissive, and uses different UVs to sample the right part.
    #ifdef USE_EMISSIVE
    vec3 emissiveComponent = texture2D(u_diffuseMap, v_emissiveCoords).rgb;
    #endif
